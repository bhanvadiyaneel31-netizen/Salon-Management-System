...
// ✅ FIX STB-012: generate a random password per seed user
const generatePassword = async () => {
  const raw = crypto.randomBytes(12).toString('base64').slice(0, 16);
  const salt = await bcrypt.genSalt(10);
  return { raw, hash: await bcrypt.hash(raw, salt) };
};

async function seedDemoData() {
  // ✅ unique random password per user — never a shared known password
  const customerPass = await generatePassword();
  const staffPass = await generatePassword();
  const adminPass = await generatePassword();
  const emmaPass = await generatePassword();
  const lisaPass = await generatePassword();

  console.log('🔑 SEED CREDENTIALS (save these now — shown only once):');
  console.log(`   customer@example.com → ${customerPass.raw}`);
  console.log(`   staff@example.com    → ${staffPass.raw}`);
  console.log(`   admin@example.com    → ${adminPass.raw}`);
  console.log(`   emma@salon.com       → ${emmaPass.raw}`);
  console.log(`   lisa@salon.com       → ${lisaPass.raw}`);

  const [customer, sarah, admin, emma, lisa] = await User.insertMany([
    { name: 'John Customer', email: 'customer@example.com', passwordHash: customerPass.hash, phone: '+1 (555) 123-4567', role: 'customer', loyaltyPoints: 100 },
    { name: 'Sarah Staff', email: 'staff@example.com', passwordHash: staffPass.hash, phone: '+1 (555) 234-5678', role: 'staff' },
    { name: 'Admin User', email: 'admin@example.com', passwordHash: adminPass.hash, phone: '+1 (555) 345-6789', role: 'admin' },
    { name: 'Emma Wilson', email: 'emma@salon.com', passwordHash: emmaPass.hash, phone: '+1 (555) 456-7890', role: 'staff' },
    { name: 'Lisa Davis', email: 'lisa@salon.com', passwordHash: lisaPass.hash, phone: '+1 (555) 567-8901', role: 'staff' },
  ]);
...
async function initDb() {
  ...
  if (!uri) {
    throw new Error('❌ MONGODB_URI is not set in environment. Add your MongoDB Atlas connection string.');
  }
  ...
  const seedAllowed = process.env.SEED_DATA === 'true' && process.env.ALLOW_SEED === 'true';

  if (userCount === 0 && (!isProduction || seedAllowed)) {
    // ✅ FIX STB-012: require TWO explicit flags to seed in production
    if (isProduction) {
      console.warn('⚠️  WARNING: Seeding demo data in PRODUCTION. This should only happen once.');
    }
    console.log('📦 Database empty — seeding demo data...');
    await seedDemoData();
  } else if (userCount === 0 && isProduction && !seedAllowed) {
    throw new Error('FATAL: Database is empty in production. Set SEED_DATA=true AND ALLOW_SEED=true to seed, or populate manually.');
  }
