...
export function CustomerDashboard({ 
  ...
  useEffect(() => {
    const loadAllData = async () => {
      ...
      await Promise.all([loadAppointments(), loadNotifications(), loadProfile()]);
    };
    loadAllData();
  ...
  const cancelAppointment = async (id: string) => {
    if (window.confirm("Are you sure you want to cancel this appointment?")) {
      try {
        await appointmentsAPI.updateStatus(Number(id), 'cancelled');
  ...
  const submitReschedule = async () => {
    ...
    try {
      ...
      await appointmentsAPI.reschedule(Number(appointmentToReschedule.id), formattedDate, rescheduleTime);
  ...
  const submitReview = async () => {
    try {
      await appointmentsAPI.submitReview(Number(appointmentToReview.id), reviewRating, reviewText);
      setAppointments(prev => prev.map(apt => 
